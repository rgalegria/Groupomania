...
exports.getAllPosts = (req, res, next) => {
  ...
  const query = db.query(sql, (error, posts) => {
    ...
    const sql2 =
      "SELECT * FROM reactions WHERE Posts_id IN (?) AND Users_id = ? ";
...
exports.getMostLikedPosts = (req, res, next) => {
  ...
  const query = db.query(sql, (error, posts) => {
    ...
    const secondSql = `SELECT * FROM reactions WHERE Posts_id IN (${postsList}) AND Users_id = ${user_id}`;
    const secondQuery = db.query(secondSql, (error, likes) => {
      if (error) throw error;
      posts.forEach((post) => {
        likes.some((like) => {
          if (like.Posts_id == post.id) {
...
exports.getOnePost = (req, res, next) => {
  ...
  const secondSql =
    "SELECT reaction FROM reactions WHERE Users_id = ? AND Posts_id = ?";
  const thirdSql =
